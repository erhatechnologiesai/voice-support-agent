# Voice Support Assistant Pipeline
...
A low-latency conversational voice support architecture integrating Speech-to-Text (STT) transcription, conversational reasoning, and Text-to-Speech (TTS) audio synthesis.

---

## Key Features

- **Streaming**: audio transcription pipeline structure
- **Conversational**: reasoning engine optimized for concise spoken responses
- **SSML**: (Speech Synthesis Markup Language) generation for natural inflection and pauses
- **Telephony-friendly**: audio codec support (PCM, WAV, u-law)
- **Turn-by-turn**: conversational state tracking
...
```mermaid
flowchart LR
    AudioIn([Inbound User Audio]) --> STT[Speech-to-Text Module]
    STT --> Agent[Conversational Voice Brain]
    Agent --> TTS[Text-to-Speech Module]
    TTS --> AudioOut[Outbound Synthesized Audio]
```
...
├── tests/
│   ├── __init__.py
│   └── test_voice_support.py   # Automated test suite
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/erhatechnologiesai/voice-support-agent.git
...
|---|---|---|
| `POST` | `/voice-turn` | Process audio speech transcription turn and return synthesized speech payload |

### Example Request

```bash
curl -X POST http://127.0.0.1:8000/voice-turn -H "Content-Type: application/json" -d '{"transcription": "I need to reset my online banking password", "caller_id": "+15550199"}'
```
...
```bash
python -m unittest tests/test_voice_support.py
```
